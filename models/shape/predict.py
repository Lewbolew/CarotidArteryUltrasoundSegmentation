...
partition = 'test'
shape_test = ShapeData(ROOT_DIR, partition)
...
PATH_TO_THE_WEIGHTS = 'weights/data_1_unet28375.059180.pth'
...
#         mask_to_encode = (np.arange(4) == test_masks.numpy()[...,None]).astype(float)
...
#         #     # changed_imgs = make_grid([test_imgs[i]], normalize=True, range=(0,255))
#         #     third_tensor = torch.cat((test_imgs[i].cpu(), masks_changed, converted_img), -1)
#         #     # third_tensor = torch.cat((test_imgs[i].cpu(), masks_changed, converted_img_unet, converted_img), -1)

#         #     # save_image(converted_img, 'predictions/shape_net_without_pretraining/{}'.format(str(unique_ind))+'.png')
...
#             unet_iou.append(metrics_evaluator.mean_IU(collapsed_softmax_logits.numpy()[k], test_masks.cpu().numpy()[k]))
#             unet_acc.append(metrics_evaluator.mean_accuracy(collapsed_softmax_logits.numpy()[k], test_masks.cpu().numpy()[k]))
...
#     np.save('unet_iou.npy', unet_iou)
#     np.save('unet_acc.npy', unet_acc)
